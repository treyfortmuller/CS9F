...
bool Position::operator==(Position coords) {
	...
	// TODO: are all angles in radians reduced to 0 -> 2*pi ???
	if (myRadius == coords.myRadius and myAngleInRadians == coords.myAngleInRadians) {
		return true;
	}
	return false;
}
...
bool Position::IsBetween(Position old, Position current)
{
	...
	float ccwDiff = current.myAngleInRadians - old.myAngleInRadians;
	
	if (ccwDiff <= PI && myRadius == old.myRadius == current.myRadius) {
		// precondition is satisfied
		// if cos (B - A) > cos (C - A), and cos (C - B) > cos (C - A) then angle B is between A and C
		if (cos(myAngleInRadians - old.myAngleInRadians) > cos(current.myAngleInRadians - old.myAngleInRadians) && cos(current.myAngleInRadians - myAngleInRadians) > cos(current.myAngleInRadians - old.myAngleInRadians)) {
			isBetween = true;
		}
	}
	return isBetween;
